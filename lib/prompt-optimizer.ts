...
function rawFallback(userInput: string): OptimizationResult {
  return {
...
export async function optimizeUserInput(
  ...
  if (!process.env.OPENAI_API_KEY) {
    return rawFallback(userInput);
  }

  const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

  // Secondary call: never fail the purchase flow — fall back to raw input.
  return withBackoffOrFallback(
    ...
    () => rawFallback(userInput),
    { label: "prompt-optimizer", fallbackOnAnyError: true, maxAttempts: 5 },
  );
}
