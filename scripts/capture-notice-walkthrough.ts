/**
 * Record a real mobile walkthrough of Notice Mode on the live Apex site.
 * Output: .video-ads-tmp/notice-walkthrough/ui/full-tour.webm
 *
 * Usage:
 *   npx tsx scripts/capture-notice-walkthrough.ts
 */
import fs from "fs";
...
const OUT_DIR = path.join(ROOT, ".video-ads-tmp", "notice-walkthrough", "ui");
const WIDTH = 1080;
const HEIGHT = 1920;
...
const BEATS: Beat[] = [
  ...
  { name: "02-home-notice", url: `${BASE}/`, scrollPx: 1400, holdMs: 3200 },
  {
    name: "03-notice-mode",
  ...
  {
    name: "05-engine-intake",
    url: `${BASE}/engine/pay-or-quit-notice-drafter?sample=1&focus=intake&state=TX`,
    scrollPx: 500,
    holdMs: 4000,
  },
  {
    name: "06-how-it-works",
    url: `${BASE}/how-it-works`,
    scrollPx: 350,
    holdMs: 3200,
  },
  { name: "07-go-notice", url: `${BASE}/go/notice`, scrollPx: 0, holdMs: 2500 },
];
...
async function main() {
  ...
  console.log("Recording Notice Mode walkthrough…");
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { capturedAt: new Date().toISOString(), beats: manifest, video: dest },
      null,
      2,
    ),
  );
